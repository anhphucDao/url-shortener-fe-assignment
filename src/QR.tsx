...
function QR({
  ...
  return (
    <div
      className={`${isValid ? 'fixed' : 'hidden'} w-full h-full top-0 right-0 bg-black/50 flex items-center justify-center`}
    >
      <div className="w-90 h-100 bg-white flex flex-col items-center justify-center rounded-lg p-6 relative overflow-hidden">
        ...
        <span
          className="w-7 h-7 bg-white absolute top-0 right-0 m-3 p-3 rounded-full text-center flex items-center justify-center cursor-pointer 
                        hover:bg-primary-100 hover:text-white transition-all ease-in delay-.3"
          onClick={onClose}
        >
          <img src="PNG\close.png" alt="close" className="scale-500" />
        </span>
        <div className="relative w-45 h-45 bg-gray-300 border-2 border-white flex items-center justify-center mb-4 border-10 border-gray-500 rounded-xl z-100">
          ...
          <span className="w-9 h-9 flex align-center justify-center bg-primary-500 text-white p-1 rounded-full absolute top-38 left-38 cursor-pointer hover:scale-120 transition-all ease-in delay-.3">
            <img src="PNG\vertical_align_bottom.png" alt="download" />
          </span>
        </div>
        ...
        <div className="flex align-center justify-between gap-2 w-full mt-5">
          ...
          <img
            src="PNG\content_copy.png"
            alt="copy"
            className="material-icons bg-primary-500 text-white p-2 rounded-md cursor-pointer hover:bg-primary-300"
            onClick={handleCopy}
          ></img>
        </div>
      </div>
